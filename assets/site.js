...
// ===== Contact channel modal (Gmail / WhatsApp / SMS) =====
const CONTACT_EMAIL = 'ndirangungu@gmail.com';
const WHATSAPP_NUMBER = '254721266628'; // +254 721 266628, international format for wa.me
const SMS_NUMBER = '+254721266628';
...
function buildSmsUrl(text) {
  // iOS Safari expects "&body=", Android and most others expect "?body="
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
  const separator = isIOS ? '&' : '?';
  return 'sms:' + SMS_NUMBER + separator + 'body=' + encodeURIComponent(text);
}
...
const contactModalSmsBtn = document.getElementById('contactModalSms');
...
if (contactModalSmsBtn) {
  contactModalSmsBtn.addEventListener('click', () => {
    window.location.href = buildSmsUrl(pendingSubject + '\n\n' + pendingBody);
    if (pendingOnSent) pendingOnSent();
    closeContactModal();
  });
}
